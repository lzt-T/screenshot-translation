---
name: Bai_Ze
description: 一套面向高频桌面截图翻译的识别实验台视觉系统
colors:
  primary: "oklch(0.55 0.19 281)"
  primary-dark: "oklch(0.6024 0.2054 281.12)"
  workbench-light: "oklch(0.975 0.003 285)"
  workbench-dark: "oklch(0.17 0.015 280)"
  surface-light: "oklch(0.995 0.001 285)"
  surface-dark: "oklch(0.22 0.02 280)"
  ink-light: "oklch(0.225 0.02 275)"
  ink-dark: "oklch(0.93 0.012 285)"
  border-light: "oklch(0.86 0.01 285)"
  border-dark: "oklch(0.35 0.025 280)"
typography:
  display:
...
Bai_Ze 把屏幕内容视作待观察的样本：取景符号负责聚焦，双语层负责比对，测量标签负责标记状态。界面安静但不匿名，冷灰白与墨紫灰构成工作台，识别紫以不同强调层级标记用户行动、聚焦与系统状态。

白泽不以神兽插画出现，而被抽象成识别、聚焦与洞察。密度服务高频桌面操作；装饰不得抢走截图、输入和译文的注意力。

**Key Characteristics:**

- 冷灰白与墨紫灰构成适应环境光的双主题工作面。
- 识别紫的实色形态表示主操作，低强调形态表示处理或完成状态。
...
- **识别紫信号色：** 以 `#7168F6` 为品牌锚点，浅色主题使用更深的同色相版本，深色主题使用原始品牌色；实色用于截图按钮、当前选择与关键焦点，低透明度底色与主色文字用于处理中和完成状态。

### Neutral

- **冷灰白工作面：** 浅色应用背景和侧栏使用低色度冷灰，仅保留极轻微的紫色倾向。
- **墨紫灰工作面：** 深色背景，避免纯黑造成过强反差。
- **样本表面：** 输入、译文和设置分组使用的内容层。
- **精密边界：** 分隔、输入轮廓和选区结构使用低色度边界。

**The Single Signal Rule.** 识别紫统一表示用户行动、选中、聚焦与系统状态；通过实色和低强调变体区分层级，页面不得引入无语义强调色。
...
- 主按钮在浅色主题使用深识别紫底、深色主题使用品牌识别紫底，并搭配高对比前景；44px 高、12px 圆角，按下时仅下移 1px。
...
- **Don't** 用识别紫装饰普通内容，或让状态徽标使用与主按钮同等重量的实色背景。
